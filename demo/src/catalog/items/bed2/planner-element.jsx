...
export default {
  ...
  info: {
    title: "bed2",
    tag: ['furnishing', 'furniture'],
    description: "bed2",
    image: require('./bed2.png')
  },
  ...
  render2D: function (element, layer, scene) {
    // Adjust the dimensions of the double bed
    let width = { length: 4.0, unit: 'ft' };
    let depth = { length: 3.0, unit: 'ft' };
  ...
  render3D: function (element, layer, scene) {
    ...
    //let width = { length: 0.0005, unit: 'ft' };
    //let depth = { length: 0.0002, unit: 'ft' };
    //let height = { length: 0.0003, unit: 'ft' };

    let width = { length: 0.00025, unit: 'ft' };  
    let depth = { length: 0.0001, unit: 'ft' };  
    let height = { length: 0.00015, unit: 'ft' };
    ...
    let onLoadItem = (object) => {
      ...
      //object.scale.set(newWidth / (width.length * 30), newHeight / (height.length * 30), newDepth / (depth.length * 30));

      object.scale.set(newWidth / (width.length * 200), newHeight / (height.length * 200), newDepth / (depth.length * 200));
    ...
    let mtl = require('./aida.mtl');
    let obj = require('./aida.obj');
